...
function Header({ t, page, locale, setLocale, go, menuOpen, setMenuOpen }: { t: Translation, page: Page, locale: Locale, setLocale: (locale: Locale) => void, go: (page: Page) => void, menuOpen: boolean, setMenuOpen: (open: boolean) => void }) {
  ...
  return <header className="site-header">
    <button className="brand" onClick={() => go('features')} aria-label="Floatem features"><OptimizedImage {...appIconImage} className="app-icon" sizes="27px" alt="" priority /><span>Floatem</span></button>
...
function Footer({ t, go }: { t: Translation, go: (page: Page) => void }) {
  return <footer><button className="footer-brand" onClick={() => go('features')}><OptimizedImage {...appIconImage} className="app-icon" wrapperClassName="footer-icon" sizes="28px" alt="" /><strong>Floatem</strong><span>© {new Date().getFullYear()} Floatem</span></button><div><button onClick={() => go('support')}>{t.nav.support}</button><button onClick={() => go('privacy')}>{t.nav.privacy}</button><a href={`mailto:${support.email}`}>{support.email}</a></div></footer>
}
