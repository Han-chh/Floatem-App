...
export default function FeaturesPage({ t, locale, go }: { t: Translation, locale: Locale, go: (page: 'download') => void }) {
  ...
  return <PageIntro label={t.features.label} title={t.features.title} intro={t.features.intro}>
    ...
    <section className="feature-list">{t.features.items.map(([number, title, body, scenario, detail], index) => <Reveal className={`feature-row${number === '05' ? ' feature-row-stacked' : ''}`} key={number}><span>{number}</span><div><h2>{title}</h2><p>{body}</p><p className="feature-scenario">{scenario}</p><small className={number === '03' ? 'feature-step-chain' : undefined}>{detail}</small></div><FeatureVisual image={images[index]} videoSrc={videos[index]} /></Reveal>)}</section>
  </PageIntro>
}
